                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                    <div class="footer-col row">
                        ...
                        <div class="col-sm-12 col-xs-6 col-xxs-12 no-padding-lg">
                                        ...
                                        @if (config('settings.geo_location.country_flag_activation'))
                                            @if (!empty(config('country.icode')))
                                                @if (file_exists(public_path().'/images/flags/32/'.config('country.icode').'.png'))
                                                    <li style="margin-right:8px;" class="flag-menu country-flag tooltipHere hidden-xs pull-left" data-toggle="tooltip" data-placement="{{ (config('lang.direction') == 'rtl') ? 'bottom' : 'right' }}" {!! $multiCountriesLabel !!}>
                            ...
                            <div style="clear:both"><hr style="color:#fff" /></div>
                            @if(isset($attr['countryCode']))
                                @php
                                    $country_details = App\Models\Country::where('code', $attr['countryCode'])->get()->first();
                                @endphp
                                <p style="color:#fff"> 
                                    @if($country_details->contact_number != null) 
                                        <strong>{{ t('contact') }}:</strong> {{ $country_details->contact_number }}
                                        <br />
                                    @endif
                                    @if($country_details->contact_email != null) 
                                        <strong>{{ t('email') }}:</strong> {{ $country_details->contact_email }}
                                        <br />
                                    @endif
                                    @if($country_details->contact_address != null) 
                                       <strong>{{ t('address') }}:</strong> {{ $country_details->contact_address }}
                                    @endif
                                </p>
                            @endif
